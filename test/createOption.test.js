...
const htmlWindowBody =
  `<input type="file" id="file" accept=".json"><div id="switches"></div>`;
...
Deno.test("create an option and check label content is equal to property", () => {
  ...
  const container = renderOption(property);
...
Deno.test("create an option that should be checked", () => {
  const property = "SomePropertyName";
  const container = renderOption(property, true);
